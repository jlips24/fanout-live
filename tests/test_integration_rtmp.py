...
""",
    ...
    def test_relay_forwards_one_source_to_multiple_destinations(self):
        source_port = _free_port()
        first_destination_port = _free_port()
        second_destination_port = _free_port()
...
[[destinations]]
name = "loopback-one"
url = "rtmp://127.0.0.1:{first_destination_port}/sink/one"

[[destinations]]
name = "loopback-two"
url = "rtmp://127.0.0.1:{second_destination_port}/sink/two"

[[pipelines]]
name = "loopback-one-direct"
source = "phone"
destination = "loopback-one"

[[pipelines]]
name = "loopback-two-direct"
source = "phone"
destination = "loopback-two"
""",
                encoding="utf-8",
            )
            ...
            sinks = [
                subprocess.Popen(
                    [
                        ...
                        "1",
                        "-i",
                        f"rtmp://127.0.0.1:{port}/sink/{stream}",
                        "-t",
                        "2",
                        "-f",
                        "null",
                ...
                for port, stream in (
                    (first_destination_port, "one"),
                    (second_destination_port, "two"),
                )
            ]
            controller = RelayController(config_path)
            try:
